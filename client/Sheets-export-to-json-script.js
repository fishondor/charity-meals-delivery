  ...
  let dataObject = phoneCells.map(
    cell => {
      ...
      while(!sheet.getRange(row + counter, column).isBlank()){
        ...
        let address = sheet.getRange(row + counter, column + 1).getValue()
        let description = sheet.getRange(row + counter, column + 2).getValue()
        let name = sheet.getRange(row + counter, column + 3).getValue()
        objectToReturn['pickups'].push({
          phone: phone,
          address: address,
